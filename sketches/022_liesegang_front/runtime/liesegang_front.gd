...
func _update_source_simulation(delta: float) -> void:
    ...
    for i: int in range(_reservoir_pos.size()):
        ...
        _front_radius[i] += front_speed * diffusion * delta * local_boost
        ...
        if _front_radius[i] > 860.0:
            _front_radius[i] = 10.0
            _next_band_radius[i] = 30.0 + hash01(float(i) * 19.1 + sketch_time) * 18.0
            _reservoir_phase[i] += 0.73
...
func _precipitate_band(owner: int) -> void:
    ...
    _band_strength.append(clampf(0.48 + supersaturation * 0.35 + nucleation * 0.18, 0.0, 1.0))
...
func _get_custom_live_debug_state() -> Dictionary:
    return {"reservoirs": _reservoir_pos.size(), "bands": _band_radius.size()}
...
func _draw() -> void:
    ...
    for r: int in range(_reservoir_pos.size()):
        ...
        source_color.a = 0.07 + (0.08 if pointer_down else 0.0)
