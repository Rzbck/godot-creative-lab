...
func _draw() -> void:
    ...
    var base_radius := min(size.x, size.y) * 0.24
    var outer_radius := min(size.x, size.y) * 0.37

    for i in point_count:
        var t := float(i) / float(point_count)
        var a := t * TAU

        var orbit_a := a + _time * (0.45 + t * 0.65)
        var orbit_b := -a * 1.7 + _time * (0.85 + t * 0.35)

        var radius_mix := 0.5 + 0.5 * sin(_time * 0.9 + a * 3.0)
        var radius := lerp(base_radius, outer_radius, radius_mix)

        var x := cos(orbit_a) * radius
        var y := sin(orbit_b) * radius * 0.72
    ...
    for i in point_count:
        for j in range(i + 1, point_count):
            ...
            if d <= connection_distance:
                var alpha := 1.0 - (d / connection_distance)
                var c := LINE.lerp(LINE_FAINT, clamp(1.0 - alpha, 0.0, 1.0))
                c.a = lerp(0.04, 0.28, alpha)
                draw_line(p1, p2, c, max(1.0, alpha * 2.2), true)

    for i in point_count:
        var pulse := 0.5 + 0.5 * sin(_time * 2.8 + float(i) * 0.37)
        var r := point_radius + pulse * 1.4
        var color := POINT.lerp(POINT_FAINT, clamp(1.0 - pulse, 0.0, 1.0))
        color.a = lerp(0.45, 0.95, pulse)
