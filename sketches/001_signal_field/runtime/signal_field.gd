...
var _time: float = 0.0
var _pointer_position: Vector2 = DESIGN_SIZE * 0.5
...
func _input(event: InputEvent) -> void:
    ...
    if event is InputEventMouseMotion:
        ...
        apply_external_pointer(motion.position, viewport_size, _pointer_down, true)
        return
...
func apply_external_pointer(
    ...
    _pointer_position = _surface_to_design(surface_position, surface_size)
...
func surface_to_design(point: Vector2, surface_size: Vector2) -> Vector2:
    return _surface_to_design(point, surface_size)
...
func get_parameter_value(parameter_id: String) -> Variant:
    match parameter_id:
        "point_count": return point_count
        "connection_distance": return connection_distance
        "point_radius": return point_radius
        "speed": return speed
        "drift": return drift
        "interaction_strength": return interaction_strength
        "interaction_radius": return interaction_radius
        "show_center_ring": return show_center_ring
        _: return null


func set_parameter_value(parameter_id: String, value: Variant) -> void:
    match parameter_id:
        "point_count": point_count = clampi(int(value), 8, 256)
        "connection_distance": connection_distance = clampf(float(value), 20.0, 320.0)
        "point_radius": point_radius = clampf(float(value), 2.0, 20.0)
        "speed": speed = clampf(float(value), 0.0, 2.0)
        "drift": drift = clampf(float(value), 0.0, 1.0)
        "interaction_strength": interaction_strength = clampf(float(value), 0.0, 180.0)
        "interaction_radius": interaction_radius = clampf(float(value), 40.0, 420.0)
        "show_center_ring": show_center_ring = bool(value)
        _: return
    queue_redraw()
...
func _draw() -> void:
    ...
    if show_center_ring:
        draw_arc(_design_to_viewport(center, origin, draw_scale), base_radius * 1.06 * draw_scale, 0.0, TAU, 128, Color(1, 1, 1, 0.05), maxf(1.0, draw_scale), true)
        draw_arc(_design_to_viewport(center, origin, draw_scale), outer_radius * 0.98 * draw_scale, 0.0, TAU, 128, Color(1, 0.72, 0.28, 0.08), maxf(1.0, draw_scale), true)
    ...
    for i: int in point_count:
        for j: int in range(i + 1, point_count):
            ...
            if distance <= connection_distance:
                ...
                draw_line(_design_to_viewport(p1, origin, draw_scale), _design_to_viewport(p2, origin, draw_scale), color, maxf(1.0, alpha * 2.2 * draw_scale), true)
    ...
    for i: int in point_count:
        ...
        draw_circle(_design_to_viewport(points[i], origin, draw_scale), maxf(0.75, radius * draw_scale), color)
...
func _surface_to_design(point: Vector2, surface_size: Vector2) -> Vector2:
    var transform_data: Dictionary = _design_transform(surface_size)
